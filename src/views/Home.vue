<template>
  <div v-if="dataLoaded" class="container mt-10 px-4">
    <!-- No Data -->
    <div v-if="data.length === 0" class="w-full flex flex-col item-center">
    ...
    >
      <router-link
        class="flex flex-col item-center bg-light-grey p-8 shadow-md cursor-pointer"
        :to="{ name: '' }"
      ...
      >
        ...
        />
        <p
          class="mt-6 py-1 px-3 text-xs text-white bg-at-light-green shadow-md rounded"
        >
          {{ workout.workoutType }}
        </p>
